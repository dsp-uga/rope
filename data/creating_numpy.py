...
import csv
import cv2
a=glob.glob('/home/rdey/dsp_final/train/*.jpg')
X_train=[]
y_train=[]
counter=0
...
for i in range (0,len(a)):
    ...
    try:
        ...
        temp_x=cv2.imread(('/home/rdey/dsp_final/train/'+str(a[i].replace('/home/rdey/dsp_final/train/','').strip())),1)
        

        try:

                
            y_train.append(csv_file[str(a[i].replace('/home/rdey/dsp_final/train/','').replace('.jpg',''))])
        ...
        if(i%10000==0 and to_skip==1):
            np.save('/home/rdey/dsp_final/train_np/X_train'+str(counter)+'.npy',np.array(X_train))
            np.save('/home/rdey/dsp_final/train_np/y_train'+str(counter)+'.npy',np.array(y_train))
            counter+=1
            X_train=[]
            y_train=[]
