...
for k in range(0,4):
	...
	for i in range(0,14):
	  ...
	  X_train=np.load(("/home/rdey/DSP_FINAL_NETWORK/train_np_90k_64/X_"+str(i)+".npy"))
	  X1_train=np.load("/home/rdey/DSP_FINAL_NETWORK/clustering_out/train_X_"+str(i)+".npy")
	  X2_train=np.load("/home/rdey/DSP_FINAL_NETWORK/object_classification_out/train_X_"+str(i)+".npy")
	  X_train = X_train.astype('float32')
	  X1_train=X1_train.astype('float32')
	  X2_train=X2_train.astype('float32')

	  #X_train=X_train.reshape(X_train.shape+(1,))
	  y_train=np.load(("/home/rdey/DSP_FINAL_NETWORK/train_np_90k_64/y_"+str(i)+".npy"))#.reshape(X_train.shape)
	  y_train=keras.utils.to_categorical(y_train, 15000)

	  model.fit([X_train,X1_train,X2_train], [y_train],
	                    batch_size=64,
	                    nb_epoch=1,
