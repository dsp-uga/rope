...
if __name__ == "__main__":
    ...
    # load dataset
    x = np.load((x_train_path + "X_train1.npy"))
    # X_train = X_train.astype('float32')

    x= x.reshape( x.shape[0], 64*64*3 )

    y = np.load((x_train_path + "y_train1.npy"))  # .reshape(X_train.shape)

    y = keras.utils.to_categorical(y, 15000)
    ...
    if args.ae_weights is None:
        dec.pretrain(x=x, y=y, optimizer=pretrain_optimizer,
                     epochs=160 , batch_size=args.batch_size,
    ...
    y_pred = dec.fit(x, y=y, tol=args.tol, maxiter=200, batch_size=args.batch_size,
